  ...
  const handleRemoveGenerated = useCallback(() => {
    ...
    for (const [classId, classData] of Object.entries(newDatabase.classes)) {
      for (const [trackId, track] of Object.entries(classData.tracks)) {
        ...
        // Remove AI levels explicitly marked as generated (samplesCount === 0)
        for (const aiLevelStr of Object.keys(track.ailevels)) {
          const aiLevel = Number(aiLevelStr);
          // Remove AI levels with samplesCount=0, which indicates they were generated by this tool
        ...
        // Recalculate min/max AI levels for the track after removal
        const aiLevels = Object.keys(track.ailevels).map(Number);
      ...
      // Recalculate min/max AI levels for the class based on all remaining tracks
      const allTrackAIs = Object.values(classData.tracks).flatMap(t => 
        Object.keys(t.ailevels).map(Number)
      );
    ...
    // Show result: either no entries removed, or detailed breakdown by class/track
    if (removedCount === 0) {
      alert('No generated AI levels found to remove');
      return;
    }

    // Build detailed report of removals per class and track using human-readable names
    const lines: string[] = [];
